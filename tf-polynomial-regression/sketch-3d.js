// Use tensorflow.js to solve y = ax^3 + bx^2 + cx + d
...
function setup() {
  ...
  d = tf.variable(tf.scalar(random(-1, 1))); // random between -1 & 1
}

function loss(pred, labels) {
  return pred.sub(labels).square().mean();
}
// basically, the equation we're evaluating.
function predict(x_array) {
  ...
  const ys = xs.pow(tf.scalar(3)).mul(a)
               ...
               .add(xs.mul(c))
               .add(d);
  return ys; // a tensor is returned
}
